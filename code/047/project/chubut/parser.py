...
@log_decorator
def alpha(get_string: Generator[str, None, None], constants: Doc, variables: Doc) -> bool:
    # INSTITUTO DE SEGURIDAD SOCIAL Y SEGUROS
    string = next(get_string)
    return string == "INSTITUTO DE SEGURIDAD SOCIAL Y SEGUROS"

@log_decorator    
def bravo(get_string: Generator[str, None, None], constants: Doc, variables: Doc) -> bool:
    # PROVINCIA DEL CHUBUT
    string = next(get_string)    
    return string == "PROVINCIA DEL CHUBUT"

@log_decorator
def charlie(get_string: Generator[str, None, None], constants: Doc, variables: Doc) -> bool:
    # Periodo: mes año
    string = next(get_string)    
    return extract_prefix(constants, string, "Periodo: ", "periodo")

@log_decorator
def delta(get_string: Generator[str, None, None], constants: Doc, variables: Doc) -> bool:
    # Apellido y Nombres: APELLIDO NOMBRE1 NOMBRE2
    string = next(get_string)    
    return extract_prefix(constants, string, "Apellido y Nombres: ", "apellido_y_nombres")        

@log_decorator    
def echo(get_string: Generator[str, None, None], constants: Doc, variables: Doc) -> bool:
    # Ley: 1820/00Banco: B.CH.CA-COMODORO RIVADAVIA
    string = next(get_string)    
    return split(constants, string, "Banco: ", ["ley", "banco"])        

@log_decorator    
def foxtrot(get_string: Generator[str, None, None], constants: Doc, variables: Doc) -> bool:
    # Tipo y Nro. Doc.: D.N.I. DDDDDDDD
    string = next(get_string)    
    return extract_prefix(constants, string, "Tipo y Nro. Doc.: ", "documento")        

@log_decorator
def golf(get_string: Generator[str, None, None], constants: Doc, variables: Doc) -> bool:
    # % Jub.: 82.00%Nro. Beneficio: DDDDDNro. Recibo: DDDD
    string = next(get_string)    
    return split(constants, string, "Nro. ", ["jub", "beneficio", "recibo"])

@log_decorator
def hotel(get_string: Generator[str, None, None], constants: Doc, variables: Doc) -> bool:
    # Categoria: 020-GABINETISTA CON 20 HS. SEM
    string = next(get_string)    
    return extract_prefix(constants, string, "Categoria: ", "categoria")

@log_decorator
def india(get_string: Generator[str, None, None], constants: Doc, variables: Doc) -> bool:
    string = next(get_string)    
...
@log_decorator
def juliet(get_string: Generator[str, None, None], constants: Doc, variables: Doc) -> bool:
    # TOTALES Hab.: Desc.: Sal. Fam. :
    # Neto Efectivo:$ DDD,DDD.23 $ DD,DDD.DD $ D,DDD.DD
    string = next(get_string)    
    running = len(string)
    if not running:
        target.error = "juliet - 1: unexpected string"
        return False
    while string != "TOTALES Hab.: Desc.: Sal. Fam. :":
        ...
        if not running:
            ...
            return False
    if not running:
        target.error = "juliet - 3: not running"
        return False
    ...
    running = len(parts) == 2
    if not running:
        target.error = "juliet - 4: len(parts) != 2"
        return False
    suffix = parts[1] 
    divisor = "$"
    ...
    running = len(parts) == 4
    if not running:
        target.error = "juliet - 5: len(parts) != 4"
        return False
...
@log_decorator
def kilo(get_string: Generator[str, None, None], constants: Doc, variables: Doc) -> bool:
    # $ 514,833.33
    string = next(get_string)    
